...
const _routes = [
  {
    ...
    component: () => import('./index.56b102bf.mjs').then((m) => m.default || m)
  }
];
...
const plugins_socketclient_ts_ssFYL5iDMi = defineNuxtPlugin(() => {
  const socket = io$1("https://63b98bffcc26af2b65da6448--dreamy-taiyaki-856c76.netlify.app:3001");
  return {
    provide: {
      io: socket
    }
...
const _sfc_main = {
  ...
  setup(__props) {
    const ErrorComponent = defineAsyncComponent(() => import('./error-component.85481129.mjs').then((r) => r.default || r));
    const nuxtApp = useNuxtApp();
