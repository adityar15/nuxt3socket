...
const _routes = [
  {
    ...
    component: () => import('./index.c112bb2e.mjs').then((m) => m.default || m)
  }
];
...
const plugins_socketclient_ts_ssFYL5iDMi = defineNuxtPlugin(() => {
  const socket = io$1("http://localhost:3001");
  return {
    provide: {
      io: socket
    }
...
const _sfc_main = {
  ...
  setup(__props) {
    const ErrorComponent = defineAsyncComponent(() => import('./error-component.74374796.mjs').then((r) => r.default || r));
    const nuxtApp = useNuxtApp();
